...
def auth_home():
    ...
    if request.method=='POST':        
        ...
        mm='%'+getval+'%'
        mycursor.execute("SELECT count(*) FROM fm_fisher where name like %s or uname like %s order by id desc",(mm,mm))
        cnt1 = mycursor.fetchone()[0]
        if cnt1>0:
            st="1"
            mycursor.execute("SELECT * FROM fm_fisher where name like %s or uname like %s order by id desc",(mm,mm))
            data1 = mycursor.fetchall()

    else:
        mycursor.execute("SELECT count(*) FROM fm_fisher order by id desc")
        cnt1 = mycursor.fetchone()[0]
        if cnt1>0:
            st="1"
            mycursor.execute("SELECT * FROM fm_fisher order by id desc")
            data1 = mycursor.fetchall()
    ...
    return render_template('auth_home.html',msg=msg,act=act,data=data,data1=data1,data4=data4,st=st)
...
def auth_request():
    ...
    if request.method=='POST':        
        getval=request.form['getval']
        mm='%'+getval+'%'
        mycursor.execute("SELECT count(*) FROM fm_trip where fisher_id like %s order by id desc",(mm,))
        cnt1 = mycursor.fetchone()[0]
        if cnt1>0:
            st="1"
            mycursor.execute("SELECT * FROM fm_trip where fisher_id like %s order by id desc",(mm,))
            data1 = mycursor.fetchall()

    else:
        mycursor.execute("SELECT count(*) FROM fm_trip")
    ...
    return render_template('auth_request.html',msg=msg,act=act,data=data,data1=data1,st=st)
